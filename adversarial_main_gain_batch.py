...
_SAMPLE_VIDEO_FRAMES =250# 90 #90 #250 #90 #79
...
def generate_input_fn(file_names, mode=tf.estimator.ModeKeys.EVAL, batch_size=1):
    def _input_fn():
        ...
        dataset = tf.data.TFRecordDataset(filenames=tf_record_list, num_parallel_reads=1)
...
def main(unused_argv):
    ...
    destribution = tf.contrib.distribute.MirroredStrategy() #devices=["/gpu:0", "/gpu:1"]
    run_config = tf.estimator.RunConfig(train_distribute=destribution)
    ...
    train_spec = tf.estimator.TrainSpec(
        input_fn =generate_input_fn('data/tfrecords/dbg_train_tfrecords.txt',mode=tf.estimator.ModeKeys.TRAIN),
        max_steps=6000,
        hooks=None
    )
    estimator.train(generate_input_fn('data/tfrecords/dbg_train_tfrecords.txt',mode=tf.estimator.ModeKeys.TRAIN))
    sess = tf.Session()
    with tf.variable_scope(scope):
