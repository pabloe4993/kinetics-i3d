...
class Unit3D(snt.Module):
  ...
  def __init__(self, output_channels,
    ...
    if self._use_batch_norm:
      self.bn = snt.BatchNorm(create_offset=True, create_scale=False)

    self.Conv3D = snt.Conv3D(output_channels=self._output_channels,
  ...
  def __call__(self, inputs, is_training):
    ...
    if self._use_batch_norm:
        net = self.bn(net,is_training=is_training, test_local_stats=False)
...
class MixUnit(snt.Module):
  """Basic unit containing Conv3D + BatchNorm + non-linearity."""

  def __init__(self, output_channels,
               end_point):
    """Initializes Unit3D module."""
    super(MixUnit, self).__init__(name='MixUnit')
    self._output_channels = output_channels
    ...
    # end_point = 'Mixed_4c'
    with tf.compat.v1.variable_scope(self.end_point):
      with tf.compat.v1.variable_scope('Branch_0'):
        self.branch_0 = Unit3D(output_channels=output_channels[0], kernel_shape=[1, 1, 1],
                          name='Conv3d_0a_1x1')
      with tf.compat.v1.variable_scope('Branch_1'):
        self.branch_1_0 = Unit3D(output_channels=output_channels[1], kernel_shape=[1, 1, 1],
                          name='Conv3d_0a_1x1')
        self.branch_1_1 = Unit3D(output_channels=output_channels[2], kernel_shape=[3, 3, 3],
                          name='Conv3d_0b_3x3')
      with tf.compat.v1.variable_scope('Branch_2'):
        self.branch_2_0 = Unit3D(output_channels=output_channels[3], kernel_shape=[1, 1, 1],
                          name='Conv3d_0a_1x1')
        self.branch_2_1 = Unit3D(output_channels=output_channels[4], kernel_shape=[3, 3, 3],
                          name='Conv3d_0b_3x3')
      with tf.compat.v1.variable_scope('Branch_3'):
        self.branch_3_0 = tf.keras.layers.MaxPool3D(pool_size=[3, 3, 3],
                                    strides=[1, 1, 1], padding='SAME',
                                    name='MaxPool3d_0a_3x3')
        self.branch_3_1 = Unit3D(output_channels=output_channels[5], kernel_shape=[1, 1, 1],
                          name='Conv3d_0b_1x1')
  ...
  def __call__(self, inputs, is_training):
    ...
    branch_0 = self.branch_0(inputs, is_training=is_training)

    branch_1_0 = self.branch_1_0(inputs, is_training=is_training)
    branch_1_1 = self.branch_1_1(branch_1_0, is_training=is_training)

    branch_2_0 = self.branch_2_0(inputs, is_training=is_training)
    branch_2_1 = self.branch_2_1(branch_2_0, is_training=is_training)

    branch_3_0 = self.branch_3_0(inputs)
    branch_3_1 = self.branch_3_1(branch_3_0, is_training=is_training)

    output = tf.concat([branch_0, branch_1_1, branch_2_1, branch_3_1], 4)
...
class InceptionI3d(snt.Module):
  ...
  def __init__(self, num_classes=400, spatial_squeeze=True,
               final_endpoint='Logits', name='inception_i3d', dropout_keep_prob=1.0):
    ...
    self.Conv3d_1a_7x7 = Unit3D(output_channels=64, kernel_shape=[7, 7, 7],
                                stride=[2, 2, 2], name=end_point)

    end_point = 'MaxPool3d_2a_3x3'
    self.MaxPool3d_2a_3x3 = tf.keras.layers.MaxPool3D(pool_size=[1, 3, 3],
                                                      strides=[1, 2, 2], padding='SAME', name=end_point)
    end_point = 'Conv3d_2b_1x1'
    self.Conv3d_2b_1x1 = Unit3D(output_channels=64, kernel_shape=[1, 1, 1],
                                name=end_point)

    end_point = 'Conv3d_2c_3x3'
    self.Conv3d_2c_3x3 = Unit3D(output_channels=192, kernel_shape=[3, 3, 3],
                                name=end_point)
    ...
    end_point = 'Mixed_3b'
    self.Mixed_3b = MixUnit(output_channels=[64, 96, 128, 16, 32, 32], end_point=end_point)

    end_point = 'Mixed_3c'
    self.Mixed_3c = MixUnit(output_channels=[128, 128, 192, 32, 96, 64], end_point=end_point)


    end_point = 'MaxPool3d_4a_3x3'
    self.MaxPool3d_4a_3x3 =  tf.keras.layers.MaxPool3D(pool_size=[ 3, 3, 3], strides=[2, 2, 2],
                           padding='SAME', name=end_point)

    end_point = 'Mixed_4b'
    self.Mixed_4b = MixUnit(output_channels=[192, 96, 208, 16, 48, 64], end_point=end_point)

    end_point = 'Mixed_4c'
    self.Mixed_4c = MixUnit(output_channels=[160, 112, 224, 24, 64, 64], end_point=end_point)

    end_point = 'Mixed_4d'
    self.Mixed_4d = MixUnit(output_channels=[128, 128, 256, 24, 64, 64], end_point=end_point)

    end_point = 'Mixed_4e'
    self.Mixed_4e = MixUnit(output_channels=[112, 144, 288, 32, 64, 64], end_point=end_point)
      
    end_point = 'Mixed_4f'
    self.Mixed_4f = MixUnit(output_channels=[256,160,320,32,128,128], end_point=end_point)


    end_point = 'MaxPool3d_5a_2x2'
    self.MaxPool3d_5a_2x2 =  tf.keras.layers.MaxPool3D(pool_size=[2, 2, 2], strides=[2, 2, 2],
                           padding='SAME', name=end_point)

    end_point = 'Mixed_5b'
    self.Mixed_5b = MixUnit(output_channels=[256,160,320,32,128,128], end_point=end_point)
      
    end_point = 'Mixed_5c'
    self.Mixed_5c = MixUnit(output_channels=[384,192,384,48,128,128], end_point=end_point)

    end_point = 'Logits'
    with tf.compat.v1.variable_scope(end_point):
        self.LogitsAvgPoll = tf.keras.layers.AveragePooling3D( pool_size=[ 2, 7, 7],
                             strides=[1, 1, 1], padding='VALID')

        self.LogitsDropout = tf.keras.layers.Dropout(dropout_keep_prob)

        self.logitsUnit3D = Unit3D(output_channels=self._num_classes,
                      kernel_shape=[1, 1, 1],
                      activation_fn=None,
  ...
  def __call__(self, inputs, is_training):
    ...
    end_point = 'Conv3d_1a_7x7'
    net = self.Conv3d_1a_7x7(net, is_training=is_training)
    ...
    end_point = 'Conv3d_2b_1x1'
    net = self.Conv3d_2b_1x1(net, is_training=is_training)
    end_points[end_point] = net
    if self._final_endpoint == end_point: return net, end_points

    end_point = 'Conv3d_2c_3x3'
    net = self.Conv3d_2c_3x3(net, is_training=is_training)
    end_points[end_point] = net
    if self._final_endpoint == end_point: return net, end_points
    ...
    end_point = 'Mixed_3b'
    net = self.Mixed_3b(net,is_training)
    end_points[end_point] = net
    if self._final_endpoint == end_point: return net, end_points

    end_point = 'Mixed_3c'
    net = self.Mixed_3c(net,is_training)
    end_points[end_point] = net
    if self._final_endpoint == end_point: return net, end_points
    ...
    end_point = 'Mixed_4b'
    net = self.Mixed_4b(net, is_training)
    end_points[end_point] = net
    if self._final_endpoint == end_point: return net, end_points

    end_point = 'Mixed_4c'
    net = self.Mixed_4c(net, is_training)
    end_points[end_point] = net
    if self._final_endpoint == end_point: return net, end_points

    end_point = 'Mixed_4d'
    net = self.Mixed_4d(net, is_training)
    end_points[end_point] = net
    if self._final_endpoint == end_point: return net, end_points

    end_point = 'Mixed_4e'
    net = self.Mixed_4e(net, is_training)
    end_points[end_point] = net
    if self._final_endpoint == end_point: return net, end_points

    end_point = 'Mixed_4f'
    net = self.Mixed_4f(net, is_training)
    end_points[end_point] = net
    if self._final_endpoint == end_point: return net, end_points
    ...
    end_point = 'Mixed_5b'
    net = self.Mixed_5b(net, is_training)
    end_points[end_point] = net
    if self._final_endpoint == end_point: return net, end_points

    end_point = 'Mixed_5c'
    net = self.Mixed_5c(net, is_training)
    end_points[end_point] = net
    if self._final_endpoint == end_point: return net, end_points

    # with tf.compat.v1.variable_scope(end_point):
    net = self.LogitsAvgPoll(net)

    net = self.LogitsDropout(net)

    net = self.logitsUnit3D(net,is_training)
